...
class PokemonDetail extends Component {
  ...
  render() {
    ...
    function catchPokemon() {
      if (Math.random() >= 0.5) {
        return prompt(
          "You caught " + name + ". Please Enter A Nickname For This Pokemon"
        );
      } else {
        ...
        {({ updatepokemon }) => (
          <Container fluid={true} className="py-5">
            <Container id="pokemonDetail">
              ...
              <Row className="justify-content-center mb-4">
                <Col sm={{ size: 8 }} md={{ size: 6 }} lg={{ size: 4 }}>
                  <h1 className="display-4 fw-bold text-light text-center text-capitalize">
                    {name}
                  </h1>
                </Col>
              </Row>
              <Row className="justify-content-center mb-4">
                <Col sm={{ size: 8 }} md={{ size: 6 }} lg={{ size: 4 }}>
                  <img
                    ...
                    src={"/pokemon/" + name + ".jpg"}
                    title={
                      "Pokemon - " + name + " - " + process.env.REACT_APP_TITLE
                    }
                    alt={
                      "Pokemon - " + name + " - " + process.env.REACT_APP_TITLE
                    }
                      ...
                      <ul>
                        {pokemon_details.stats
                          ? pokemon_details.stats.map((item) => {
                              const { stat } = item;
                              return (
                                <li className="text-capitalize" key={stat.name}>
                                  {stat.name}
                                </li>
                              );
                            })
                      ...
                      <ul>
                        {pokemon_details.types
                          ? pokemon_details.types.map((item) => {
                              const { type } = item;
                              return (
                                <li className="text-capitalize" key={type.name}>
                                  {type.name}
                                </li>
                              );
                            })
                      ...
                      <ul>
                        {pokemon_details.held_items
                          ? pokemon_details.held_items.map((item_hi) => {
                              const { item } = item_hi;
                              return (
                                <li className="text-capitalize" key={item.name}>
                                  {item.name}
                                </li>
                              );
                            })
                      ...
                      <ul>
                        {pokemon_details.moves
                          ? pokemon_details.moves.map((item) => {
                              const { move } = item;
                              return (
                                <li className="text-capitalize" key={move.name}>
                                  {move.name}
                                </li>
                              );
                            })
